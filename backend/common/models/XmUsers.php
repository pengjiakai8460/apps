 ...
 * @property string $openid 微信的openid，在录播课微信端支付时加入，与wechat_openid字段互不影响使用，不存在关系
 */
class XmUsers extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['account', 'password', 'name', 'intention_level', 'xmschool_id', 'adder_id', 'add_time', 'update_time'], 'required'],
            [['employee_id', 'birthday', 'school_id', 'admission_year', 'xmschool_id', 'market_id', 'market_time', 'cc_id', 'recommend_id', 'adder_id', 'add_time', 'update_time', 'allot_cc_time', 'follow_count'], 'integer'],
            [['remark'], 'string'],
            [['account'], 'string', 'max' => 40],
            [['password', 'father_name', 'mother_name', 'mother_job', 'school_duty'], 'string', 'max' => 80],
            [['name', 'nickname', 'wechat', 'active_place', 'father_job', 'father_email', 'father_wechat', 'mother_email', 'mother_wechat', 'love_level', 'father_love_level', 'mother_love_level', 'wechat_openid', 'autograph'], 'string', 'max' => 200],
            ...
            [['sex', 'from_type'], 'string', 'max' => 3],
            [['communtity_name', 'address', 'accept_level'], 'string', 'max' => 800],
            [['main_contacts', 'is_student', 'status'], 'string', 'max' => 1],
